...
2. `/vc-short:crawl-novel` 爬取小说章节 → `chapters/ch<NN>/novel.md`（也可由用户手动粘贴原文）
3. `/vc-short:extract` 提取角色/场景
4. `/vc-short:gen-character` 生成角色资产（图片+音色）
5. `/vc-short:gen-image` 生成场景/服装/道具图片
6. `/vc-short:gen-voice` 重新生成角色音色（不满意时用）
7. `/vc-short:gen-script` 改编剧本
8. `/vc-short:gen-shots` 拆分分镜
9. `/vc-short:gen-keyframe` 生成分镜首帧图（确认满意后再生成视频，避免一次确认同时产生图片+视频两笔费用）
10. `/vc-short:gen-video` 生成分镜视频（自动使用已确认的首帧图）
11. `/vc-short:compose-chapter` 合成章节视频
