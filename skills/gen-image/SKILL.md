...
>
> **角色自动追加"防写实约束"**：gen-character/gen-image 生成角色图时，脚本自动追加"面部卡通风格化，五官适度简化，皮肤无真实毛孔纹理，非写实，非真人照片感"——角色脸部过度写实会被视频 API 审核判定为真人（`InputImageSensitiveContentDetected.PrivacyInformation`）导致 gen-video 提交失败。角色图被审核拦截时，用 `/vc-short:fix-image` 把面部往风格化方向调整后重试。
